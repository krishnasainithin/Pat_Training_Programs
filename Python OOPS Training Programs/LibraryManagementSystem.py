...
if __name__ == "__main__":
    ...
    IARElibrary = Library(
        ["COMPUTER NETWORKS", "OPERATING SYSTEMS", "DATABASE MANAGEMENT SYSTEMS", "DATA STRUCTURES", "LINUX PROGRAMMING", "COMPUTER ORGANIZATION AND ARCHITECTURE"])
    student = Student()
    track = []

    print("\t\t\t\t\t\t\t♦♦♦♦♦♦♦ WELCOME TO THE IARE LIBRARY ♦♦♦♦♦♦♦\n")
    ...
    while (True):
        ...
        try:
            ...
            if usr_response == 1:  # listing
                IARElibrary.displayAvailableBooks()
            elif usr_response == 2:  # borrow
                IARElibrary.borrowBook(
                    input("Enter your name: "), student.requestBook())
            elif usr_response == 3:  # return
                IARElibrary.returnBook(student.returnBook())
            elif usr_response == 4:  # donate
                IARElibrary.donateBook(student.donateBook())
